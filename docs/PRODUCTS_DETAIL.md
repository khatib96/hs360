...
product_units (for each of the 10):
  ...
  barcode = '8901234567890' (same as parent product, optional override)
```

### 2.3 Why Barcode Also on Product Unit?

Sometimes individual units **do** have their own unique barcodes (e.g., when each unit gets a printed sticker at receipt time). The `product_units.barcode` field allows this, but defaults to the parent product's barcode.
