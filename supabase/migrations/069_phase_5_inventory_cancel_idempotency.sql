-- Phase 5 M4.5 corrective pass: cancel idempotency replay and serialized cancel guard.

alter table public.inventory_documents
  add column if not exists cancellation_idempotency_key uuid,
  add column if not exists cancellation_idempotency_payload_hash text;

create unique index if not exists ux_inventory_documents_tenant_cancel_idempotency
  on public.inventory_documents (tenant_id, cancellation_idempotency_key)
  where cancellation_idempotency_key is not null;

create or replace function public.normalize_cancel_inventory_document_payload(
  p_document_id uuid,
  p_reason text
)
returns jsonb
...
create or replace function public.compute_cancel_inventory_document_payload_hash(
  p_document_id uuid,
  p_reason text
)
returns text
...
as $$
  select encode(
    digest(
      convert_to(
        public.normalize_cancel_inventory_document_payload(p_document_id, p_reason)::text,
        'UTF8'
      ),
      'sha256'
    ),
  ...
  loop
    ...
    if exists (
      ...
      where m.tenant_id = v_tenant_id
        ...
        and m.created_at > v_doc.created_at
    ) then
      raise exception 'correction_document_required';
    end if;
  end loop;
...
comment on function public.normalize_cancel_inventory_document_payload(uuid, text) is
  'M4.5: Canonical cancel-inventory-document payload for idempotency hashing.';

comment on function public.compute_cancel_inventory_document_payload_hash(uuid, text) is
  'M4.5: SHA-256 hex of canonical cancel-inventory-document payload.';

comment on function public.cancel_inventory_document(uuid, text, uuid) is
  'M4.5: Safe inventory document cancellation with idempotent replay. Serialized documents require correction workflow.';

revoke all on function public.normalize_cancel_inventory_document_payload(uuid, text)
  from public, anon, authenticated;
revoke all on function public.compute_cancel_inventory_document_payload_hash(uuid, text)
  from public, anon, authenticated;
