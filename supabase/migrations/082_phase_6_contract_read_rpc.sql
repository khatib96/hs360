-- Phase 6 M8: contract read RPCs, field masking, and direct-table ACL hardening.
-- Clients read contracts through RPCs and safe views/column grants only.

-- ---------------------------------------------------------------------------
-- Section A: revoke broad table SELECT; grant safe columns only
-- ---------------------------------------------------------------------------
revoke all on table public.contracts from authenticated, anon;
revoke all on table public.contract_lines from authenticated, anon;
...
grant select (
  ...
  line_order,
  created_at
) on table public.contract_lines to authenticated;

-- ---------------------------------------------------------------------------
-- Section B: safe views (column subset; security_invoker preserves RLS)
-- ---------------------------------------------------------------------------
...
drop view if exists public.contract_lines_safe;

create view public.contract_lines_safe
with (security_invoker = true) as
  select
    id,
    tenant_id,
...
grant select on public.contract_lines_safe to authenticated;

-- ---------------------------------------------------------------------------
-- Section C: mask contract read/detail JSON by field permissions
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Section D: internal detail JSON builder
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_result := public.build_contract_detail_json(p_contract_id);
  return public.mask_contract_read_json(v_result);
end;
$$;
...
-- Section G: grants
-- ---------------------------------------------------------------------------
revoke all on function public.mask_contract_read_json(jsonb) from public, anon, authenticated;
revoke all on function public.build_contract_detail_json(uuid) from public, anon, authenticated;
...
comment on function public.list_contracts(uuid, text, text, date, date, text, boolean, integer, integer) is
  'Phase 6 M8: bounded contract list with party/location labels. No snapshot cost columns.';

comment on function public.get_contract_detail(uuid) is
  'Phase 6 M8: contract detail JSON with permission-masked snapshot fields.';
