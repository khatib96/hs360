...
void main() {
  group('CalendarMapAppResolver', () {
    test(
      'iOS mapped offers Apple HTTPS + Google/Waze when launchable + browser',
      () async {
        ...
        final apple = options.firstWhere(
          (o) => o.kind == CalendarMapAppKind.appleMaps,
        );
    ...
    test('Apple Maps HTTPS URI is built with Uri.https (no maps: scheme)', () {
      ...
      expect(
        uri,
        Uri.https('maps.apple.com', '/', {'daddr': '29.3759,47.9774'}),
      );
      expect(uri.scheme, isNot(equals('maps')));
    });

    test(
      'Android mapped offers system/Google/Waze when launchable + browser',
      () async {
        final resolver = CalendarMapAppResolver(canLaunch: (_) async => true);
        final options = await resolver.resolve(
          _mappedTarget,
    ...
    test(
      'hides probed apps that cannot launch; Apple Maps still offered on iOS',
      () async {
        final resolver = CalendarMapAppResolver(canLaunch: (_) async => false);
        final options = await resolver.resolve(
          _mappedTarget,
          platform: TargetPlatform.iOS,
        );
    ...
    test(
      'url_only offers Browser only; never invents coords or fake Google Maps',
      () async {
        final resolver = CalendarMapAppResolver(canLaunch: (_) async => true);
        final options = await resolver.resolve(
          _urlOnlyTarget,
  ...
  group('CalendarDirectionsLauncher', () {
    ...
    test('Android browser launches via Chrome navigate URI', () async {
      Uri? launched;
      final launcher = CalendarDirectionsLauncher(
        launcher: (uri, {mode = LaunchMode.platformDefault}) async {
          launched = uri;
      ...
      final https = Uri.parse(
        'https://www.google.com/maps?q=29.3753,47.9774&query=p7m12',
      );
      final option = CalendarMapAppOption(
        kind: CalendarMapAppKind.browser,
        uri: https,
      ...
      final result = await launcher.launch(option);
      expect(result, CalendarDirectionsResult.opened);
      expect(launched!.scheme, 'googlechrome');
      expect(
        launched.toString(),
        contains(Uri.encodeComponent(https.toString())),
      );
    });
